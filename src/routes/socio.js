...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!socios) return res.status(400).send({error: "User not founded"});
    ...
    const token = jwt.sign({userId: socios.id}, authenticate.secret, {
      expiresIn: 86400,
    });
...
router.post("/SignOn", async (req, res) => {
  ...
  try {
    ...
    socios.save();
    const token = jwt.sign({id: socios.id}, authenticate.secret, {
      expiresIn: 86400,
    });
